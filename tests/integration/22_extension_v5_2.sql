-- Extension IMAGE V5.2 : ce que l'ajout de cinquante commandes doit avoir
-- produit, et surtout ce qu'il ne doit avoir touche a rien d'autre.
--
-- Meme forme que le controle de la V5.1, et c'est voulu : deux extensions
-- qui se verifient de la meme facon se comparent d'un coup d'oeil.
--
...
declare
  v_n integer;
begin
  select count(*) into v_n from public.prompts where catalog_version = 'v5.2';

  if v_n = 0 then
    raise notice 'Extension V5.2 non appliquee : controles ignores.';
    return;
  end if;

  -- --- Ce qui est arrive ----------------------------------------------

  perform tests_assert(v_n = 50, format('%s commandes V5.2 au lieu de 50.', v_n));
  ...
  perform tests_assert(v_n = 0, format('%s commandes V5.2 servent le meme texte a deux moteurs.', v_n));
  ...
  select count(*) into v_n from public.prompts
  where catalog_version = 'v5.2' and coalesce(default_image_path, '') <> '';
  perform tests_assert(v_n = 0, format('%s commandes V5.2 portent un chemin de visuel.', v_n));
  ...
  select count(*) into v_n from public.prompts
  where catalog_version = 'v5.2' and command::text !~ '^/[a-z][a-z0-9_]{2,31}$';
  perform tests_assert(v_n = 0, format('%s commandes V5.2 hors du motif R02.', v_n));

  -- --- Cherchables des leur arrivee -------------------------------------

  select count(*) into v_n from public.prompts
  where catalog_version = 'v5.2' and coalesce(intention, '') = '';
  perform tests_assert(v_n = 0, format('%s commandes V5.2 sans intention.', v_n));

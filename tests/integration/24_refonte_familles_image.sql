...
-- refonte les regroupe en trois, dans un ordre voulu — les portraits
-- d'abord, la publicite ensuite, la technique enfin. Ce fichier controle ce
-- que l'utilisateur rencontre a l'ecran, pas la mecanique du deplacement.
--
...
begin
  ...
  where external_ref in ('IMG-V6-01', 'IMG-V6-02', 'IMG-V6-03') and is_visible;
  ...
  -- --- Trois rayons, et rien d'autre ------------------------------------

  perform tests_assert(v_n = 3, format('%s familles V6 visibles au lieu de 3.', v_n));

  select count(*) into v_n from public.categories
  where mode = 'image' and external_ref is not null and is_visible;
  perform tests_assert(v_n = 3,
  ...
  perform tests_assert(
    v_ordre = 'Portraits et effets visuels > Publicité et marques > Techniques et lieux',
  ...
  where c.external_ref in ('IMG-V6-01', 'IMG-V6-02', 'IMG-V6-03')

...
begin
  ...
  -- Le catalogue V5 n'a pas ete touche : 433 commandes, ni une de plus.
  select count(*) into v_avant from public.prompts where level is not null;
  perform tests_assert(v_avant = 483,
    format('%s commandes graduees au lieu de 483 (433 + 50).', v_avant));
